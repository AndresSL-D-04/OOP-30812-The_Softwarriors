...
public class DocumentConverter {
    ...
    public static Document userToDoc(User u) {
        ...
        if (u.getEmail() != null) doc.append("email", u.getEmail());
        if (u.getResetToken() != null) doc.append("resetToken", u.getResetToken());
        if (u.getResetTokenExpiry() != null) doc.append("resetTokenExpiry", u.getResetTokenExpiry());
        if (u.getCreatedAt() != null) doc.append("createdAt", u.getCreatedAt());
        if (u.getLastLogin() != null) doc.append("lastLogin", u.getLastLogin());
    ...
    public static User docToUser(Document doc) {
        ...
        User user = new User(username, password != null ? password : "", role != null ? role : "");
        
        user.setEmail(doc.getString("email"));
        user.setResetToken(doc.getString("resetToken"));
        user.setResetTokenExpiry(doc.getString("resetTokenExpiry"));
        user.setActive(doc.getBoolean("active", true));
        user.setCreatedAt(doc.getString("createdAt"));
        user.setLastLogin(doc.getString("lastLogin"));
